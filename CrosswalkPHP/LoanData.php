...
$row = 1;
if (($handle = fopen('DataSummaryOption/LoanData.csv', 'r')) !== FALSE){
   ...
   // $data = fgetcsv($handle, 1000, ',');
    ...
    //  if ( $data!== FALSE ){
       // echo '<tr><th class="fixed-side">'.implode('</th><th class="fixed-side">', $data).'</th></tr>';
        ...
        echo '<th class="fixed-side">Title XII Loan Balance</th>';
        echo '<th class="fixed-side">Loan Per Co. Employee</th>';
        echo '<th class="fixed-side">Loan as % of Total wages</th>';
        echo '<th class="fixed-side">Non-Title XII Loan Balance</th>';
        echo '<th class="fixed-side">Begin Date Non-Title XII Loan </th>';
        echo '</tr>';
    // }
    // Get the rest
